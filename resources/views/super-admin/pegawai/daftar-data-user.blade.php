                ...
                <div class="card bg-white">
                    ...
                    <div class="card-body overflow-scroll">
                        @if (Auth::user()->role == 'SuperAdmin')
                            <a href="{{ url('/data-user/input-user') }}" class="btn btn-primary mb-3"><i
                                    class="fa-solid fa-file-circle-plus"></i>Add User</a>
                        @endif
                        <div class="row mb-3 mx-1">
                            <form class="nav-link d-none d-lg-flex search align-items-center" action="{{ url('/data-user') }}">
                                <div class="col-6">
                                                ...
                                                <div class="modal-body">
                                                    ...
                                                    <p>Umur : <span
                                                        >{{ \Carbon\Carbon::parse($data->tanggal_lahir)->age }}
                                                            Tahun</span></p>
                                                    <p>Nomor HP : <span>{{ $data->nomor_hp }}</span></p>
                                                    <p>Status Pernikahan : <span
                                                        >{{ $data->status_pernikahan }}</span></p>
                                                    @if ($data->status_pernikahan != 'Lajang')
                                                        <p>Jumlah Anak : <span
                                                            >{{ $data->jumlah_anak }}</span></p>
                                                    @endif
                                                    <p>Department : <span>{{ $data->department }}</span>
                                                    </p>
                                                    <p>Golongan : <span>{{ $data->golongan }}</span></p>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
